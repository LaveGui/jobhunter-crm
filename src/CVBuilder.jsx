...
import { Mail, Phone, MapPin, Linkedin, Trash2, PlusCircle, Printer, ArrowLeft, LayoutTemplate, Globe, Download, ScanEye, Settings, Type, AlignJustify } from 'lucide-react';
...
export default function CVBuilder() {
  ...
  const [cv, setCv] = useState({
    ...
    personal: {
      ...
      photoUrl: "", 
      summary: "Profesional enfocado en Sales Enablement y Arquitectura de Software con experiencia liderando equipos..."
    },
    experience: [
      { id: 1, role: "Product Marketing Specialist", company: "Jeff App", date: "2022 - 2023", description: "Retención: Estrategia con Braze para base de 100k+ usuarios...\nLogro destacado: Aumento del 20% en LTV." }
    ],
    education: [
  ...
  // --- NUEVO ESTADO: CONFIGURACIÓN DE DISEÑO ---
  const [design, setDesign] = useState({
    nameSize: 32,       // Tamaño Nombre Principal
    roleSize: 14,       // Tamaño Título Puesto (Experiencia)
    companySize: 12,    // Tamaño Empresa
    textSize: 10,       // Tamaño Texto General (Descripciones)
    lineHeight: 1.4,    // Espaciado entre líneas (arregla lo del Enter)
    sectionGap: 20,     // Espacio entre experiencias
  });
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col md:flex-row font-sans">
      ...
      <style>{`
        ...
        @media print {
          ...
          /* El contenedor principal crece infinitamente y repite el fondo */
          .cv-container {
            width: 210mm !important;
            min-height: 297mm !important; /* Mínimo una hoja */
            height: auto !important;      /* Crece si hay más contenido */
            margin: 0 !important;
      ...
      <aside className="w-full md:w-[450px] bg-white h-screen overflow-y-auto border-r border-gray-200 shadow-xl z-10 print:hidden flex flex-col">
        ...
        <div className="p-4 bg-slate-900 text-white flex justify-between items-center sticky top-0 z-20">
          ...
          <div className="flex items-center gap-2">
             <button onClick={() => setDebugMode(!debugMode)} className={`text-[10px] px-2 py-1 rounded border ${debugMode ? 'bg-red-500 border-red-500' : 'border-gray-600 text-gray-400'}`}>
               <ScanEye size={14}/>
             </button>
             <button onClick={handlePrint} className="bg-blue-600 hover:bg-blue-700 px-3 py-1.5 rounded text-sm font-bold flex gap-2">
               <Printer size={16}/> PDF
             </button>
          </div>
        </div>
        ...
        <div className="flex border-b sticky top-[60px] bg-white z-10">
          <button 
            ...
            className={`flex-1 py-3 text-xs font-bold uppercase flex items-center justify-center gap-2 ${activeTab === 'content' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-400'}`}
          >
            <LayoutTemplate size={14}/> Contenido
          </button>
          <button 
            onClick={() => setActiveTab('design')} 
            className={`flex-1 py-3 text-xs font-bold uppercase flex items-center justify-center gap-2 ${activeTab === 'design' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-400'}`}
          >
            <Settings size={14}/> Diseño
          </button>
        </div>
          ...
          {activeTab === 'content' && (
            <>
              ...
              <section className="space-y-3 border-t pt-4">
                ...
                {/* CAMBIO: Ahora el resumen es más importante porque va en la barra lateral */}
              ...
              {/* EDUCACIÓN & IDIOMAS & SKILLS (Igual que antes...) */}
              <section className="space-y-4 border-t pt-4">
                <div className="flex justify-between items-center">
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="flex justify-between text-xs font-semibold text-gray-600 mb-1">
                      Nombre Principal <span>{design.nameSize}px</span>
                    </label>
                    <input type="range" min="20" max="60" value={design.nameSize} onChange={(e) => setDesign({...design, nameSize: Number(e.target.value)})} className="w-full accent-blue-600"/>
                  </div>

                  <div>
                    <label className="flex justify-between text-xs font-semibold text-gray-600 mb-1">
                      Cargo / Título Puesto <span>{design.roleSize}px</span>
                    </label>
                    <input type="range" min="10" max="24" value={design.roleSize} onChange={(e) => setDesign({...design, roleSize: Number(e.target.value)})} className="w-full accent-blue-600"/>
                  </div>

                  <div>
                    <label className="flex justify-between text-xs font-semibold text-gray-600 mb-1">
                      Empresa <span>{design.companySize}px</span>
                    </label>
                    <input type="range" min="8" max="18" value={design.companySize} onChange={(e) => setDesign({...design, companySize: Number(e.target.value)})} className="w-full accent-blue-600"/>
                  </div>

                  <div>
                    <label className="flex justify-between text-xs font-semibold text-gray-600 mb-1">
                      Texto Descripciones <span>{design.textSize}px</span>
                    </label>
                    <input type="range" min="8" max="16" value={design.textSize} onChange={(e) => setDesign({...design, textSize: Number(e.target.value)})} className="w-full accent-blue-600"/>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="flex justify-between text-xs font-semibold text-gray-600 mb-1">
                      Interlineado (Párrafos) <span>{design.lineHeight}</span>
                    </label>
                    <input type="range" min="1" max="2" step="0.1" value={design.lineHeight} onChange={(e) => setDesign({...design, lineHeight: Number(e.target.value)})} className="w-full accent-gray-600"/>
                    <p className="text-[10px] text-gray-400 mt-1">Ajusta esto si los saltos de línea son muy grandes.</p>
                  </div>

                  <div>
                    <label className="flex justify-between text-xs font-semibold text-gray-600 mb-1">
                      Separación entre Empleos <span>{design.sectionGap}px</span>
                    </label>
                    <input type="range" min="5" max="50" value={design.sectionGap} onChange={(e) => setDesign({...design, sectionGap: Number(e.target.value)})} className="w-full accent-gray-600"/>
                  </div>
                </div>
              </div>
      ...
      <main className="flex-1 bg-gray-500 overflow-y-auto p-4 md:p-8 flex justify-center print:p-0 print:bg-white">
        ...
        <div className="relative">
          {/* GUÍA DE CORTE DE PÁGINA */}
          <div className="print-hidden absolute left-0 w-full border-b-2 border-dashed border-red-400 z-50 flex items-end justify-end pointer-events-none opacity-50" 
               style={{ top: '297mm', width: '210mm' }}>
            <span className="bg-red-400 text-white text-[10px] px-2 py-0.5 rounded-t font-bold">FIN DE PÁGINA 1</span>
          </div>
          ...
          >
            ...
            <div className={`w-[32%] p-6 pt-10 flex flex-col shrink-0 text-white ${debugClass}`}>
              ...
              <div className="space-y-8 flex-1">
                ...
                {/* 1. PERFIL (MOVIDO AQUI) */}
                <div className={debugClass}>
                  <h3 className="font-bold uppercase tracking-wider mb-3 pb-1 text-xs border-b border-white/30 text-white/90 flex items-center gap-2">
                    <LayoutTemplate size={12}/> Perfil
                  </h3>
                  <p className="text-[10px] leading-relaxed text-white/90 text-justify whitespace-pre-line">
                    {cv.personal.summary}
                  </p>
                </div>
                ...
                <div className={debugClass}>
                  ...
                  {cv.education.map((edu) => (
                     <div key={edu.id} className="mb-4 text-white/90">
                       <p className="font-bold text-[10px] mb-0.5">{edu.degree}</p>
                       <p className="text-[9px] opacity-80">{edu.school}</p>
                       <p className="text-[9px] opacity-80">{edu.date}</p>
                     </div>
                  ))}
                </div>
                ...
                <div className={debugClass}>
                  ...
                  {cv.languages.map((lang) => (
                    <div key={lang.id} className="mb-2 flex justify-between items-baseline text-[10px] text-white/90">
            ...
            <div className={`w-[68%] p-8 pt-12 flex flex-col text-slate-800 ${debugClass}`}>
              ...
              <header className="mb-8 pb-4 shrink-0 border-b-2" style={{ borderColor: cv.themeColor }}>
                <h1 className="font-extrabold uppercase tracking-tight leading-none mb-2 text-slate-900" 
                    style={{ fontSize: `${design.nameSize}px` }}>
                  {cv.personal.name}
                </h1>
                <h2 className="font-bold tracking-wide" 
                    style={{ color: cv.themeColor, fontSize: '18px' }}>
                  {cv.personal.title}
                </h2>
              </header>

              {/* EXPERIENCIA (Ahora tiene más espacio porque quitamos el perfil de aquí) */}
              <section className="flex-1">
                <h3 className="text-xs font-bold uppercase tracking-wider mb-6 flex items-center gap-2 text-slate-600">
                  ...
                  <span className="mt-[1px]">Experiencia Profesional</span>
                </h3>
                
                {/* Lista de Experiencias con gap dinámico */}
