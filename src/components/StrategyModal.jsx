...
import { X, Save, RotateCcw, Clock, MessageSquare, Phone, Mail, StickyNote, Key, Eye, UserPlus } from 'lucide-react';
...
export default function StrategyModal({ isOpen, onClose, onSave }) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      else setRules(DEFAULT_PLAYBOOK);
  ...
  const handleDayChange = (index, value) => {
    const newRules = rules.map((r, i) => i === index ? { ...r, day: Number(value) } : r);
    setRules(newRules);
  };

  const toggleRule = (index) => {
    const newRules = rules.map((r, i) => i === index ? { ...r, enabled: r.enabled === false ? true : false } : r);
    setRules(newRules);
  };
  ...
  const handleReset = () => {
    if (window.confirm("¿Volver a la estrategia por defecto?")) setRules(DEFAULT_PLAYBOOK);
  };

  const getIcon = (type) => {
    switch(type) {
      case 'visit': return <Eye size={16} className="text-purple-500"/>;
      case 'connect': return <UserPlus size={16} className="text-green-500"/>;
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 p-4">
      <div className="bg-slate-900 text-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-700">
        ...
        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
          <div>
            <h3 className="text-sm font-bold text-slate-300 mb-3 flex items-center gap-2">⏱️ Cadencia de Seguimiento</h3>
            <div className="space-y-3">
              {rules.map((rule, idx) => (
                <div key={idx} className={`p-3 rounded-lg border flex gap-3 items-center transition-all ${rule.enabled === false ? 'bg-slate-800/30 border-slate-800 opacity-50' : 'bg-slate-800 border-slate-600'}`}>
                  <input type="checkbox" checked={rule.enabled !== false} onChange={() => toggleRule(idx)} className="w-4 h-4 rounded border-slate-500 bg-slate-700 cursor-pointer"/>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-0.5">{getIcon(rule.action)}<span className="font-bold text-xs">{rule.label}</span></div>
                  </div>
        ...
        <div className="p-4 border-t border-slate-700 bg-slate-950 flex justify-between">
          <button onClick={handleReset} className="px-4 py-2 text-slate-400 hover:text-white text-xs font-bold flex items-center gap-2"><RotateCcw size={14}/> Resetear</button>
          <div className="flex gap-2">
            <button onClick={onClose} className="px-4 py-2 text-slate-300 hover:text-white font-bold text-sm">Cancelar</button>
            <button onClick={handleSave} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg font-bold text-sm flex items-center gap-2"><Save size={16}/> Guardar Todo</button>
          </div>
        </div>
      </div>
    </div>
